...
{
    /*primeiro video - leitura e criar tabela de frequência*/
    unsigned int tab_frequencia[TAM];
    if (argc < 2) {
        printf("Erro: Você não informou o arquivo para compactar!\n");
        printf("Modo de uso correto: %s nome_do_arquivo.extensao\n", argv[0]);
        return 1;
    }
    ...
    readfile(nome_do_arquivo, tab_frequencia);

    /*segundo video - seria de lista encadeada, trocamos por heap*/
    /* conta quantos bytes distintos existem para dimensionar a heap*/
    int distintos = 0;
    for (int i = 0; i < TAM; i++){
        if (tab_frequencia[i] > 0) 
        {
    ...
    if (distintos == 0) 
    {
        ...
        return 0; // Finaliza o programa sem erro/crash
    }
    
    heap *h = criar_heap(distintos * 2); // * 2 pois a montagem insere nós pai de volta 
    preencher_heap(h, tab_frequencia); 


    /*terceiro video- arvore de huffman*/
    no *arvore = montar_arvore(h);

    /*video quatro- montar dicionario*/
  ...
  /* video 5 - codificar(comprimir) */
    compactar(dicionario, nome_do_arquivo, arvore, tab_frequencia);
